...
const loveStoryConfig = {
  ...
  timeline: [
    { year: "2019", label: "First Meeting" },
    { year: "❤️", label: "Fell in Love" },
    { year: "2026", label: "Forever Together" },
  ],
};
    ...
    <section className="relative py-12 sm:py-16 md:py-20 lg:py-24 bg-gradient-to-br from-orange-50 via-orange-100 to-rose-50 overflow-hidden">
      ...
      >
        ...
        <motion.div variants={textVariants} className="max-w-2xl mx-auto">
          {loveStoryConfig.paragraphs.map((para, idx) => (
            <p
              key={idx}
              className="text-sm sm:text-base md:text-lg lg:text-xl text-orange-900 leading-relaxed mb-6 sm:mb-8 px-2 sm:px-0 font-para font-semibold tracking-wide last:mb-8 sm:last:mb-10"
            >
              {para}
            </p>
          ))}
        ...
        >
          {loveStoryConfig.timeline.map((item) => (
            <div
              key={item.label}
              className="flex flex-col items-center p-4 sm:p-6 bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-orange-100/50"
            >
              ...
              <p className="text-xs sm:text-sm text-orange-800 font-medium text-center">
                {item.label}
              </p>
            </div>
          ))}
